...
const PokemonCreate = () => {
    ...
    const history = useHistory();// me redirije a la ruta 

    const [input, setInput] = useState({ // para el form
        name: '',
        hp: '',
        attack: '',
        defense: '',
    ...
    let validateImg = /.*(png|jpg|jpeg|gif)$/;
//  let validateImg =  /\.(jpg|jpeg|png)$/i;
    ...
    const validate = (input) => {
     ...
     // if (!validateUrl.test(input.url) || parseInt(input.url) < 1) {
    //        errors.url = "URL required";
    // }   
    // if (!validateImg.test(input.img) || parseInt(input.img) < 1) {
     //       errors.img = "IMG required";
   // }

   // if  (!validateUrl.test(input.url && !validateImg.test(input.img);
  //            parseInt(input.url.img) < 1) {
    //            errors.url.img = "required";
  //}
    ...
    const handleSubmit = e => {
        ...
        if (
            ...
            dispatch(postPokemon(input));// seteo el input en cero se lo paso vacio 'limpiar'
            setInput({
                name: '',
                hp: '',
                attack: '',
            ...
            history.push('/home') // me redirije a la ruta **
        } else {
